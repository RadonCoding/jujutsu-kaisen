...
public class Mimicry extends Ability implements IToggled, IAttack {
    ...
    public boolean attack(DamageSource source, LivingEntity owner, LivingEntity target) {
        ...
        CursedTechnique current = ownerSorcererData.getTechnique();
        CursedTechnique copied = targetData.getTechnique();

        if (copied == null || current == null || ownerSorcererData.hasTechnique(copied) || current == copied)
            return false;
        ...
        if (owner instanceof ServerPlayer player) {
            PacketDistributor.sendToPlayer(player, new SyncMimicryDataS2CPacket(ownerMimicryData.serializeNBT(player.registryAccess())));
        }
        return true;
    }
}
