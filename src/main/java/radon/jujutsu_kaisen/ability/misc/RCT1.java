...
public class RCT1 extends Ability implements Ability.IChannelened {
    ...
    public void run(LivingEntity owner) {
        owner.heal((float) (ConfigHolder.SERVER.healingAmount.get().floatValue() * Math.pow(2.0D, Math.log(this.getPower(owner)))));
    }

    @Override
    public float getCost(LivingEntity owner) {
        if (owner.getHealth() < owner.getMaxHealth()) {
            return (float) (ConfigHolder.SERVER.healingAmount.get().floatValue() * Math.pow(2.0D, Math.log(this.getPower(owner))) * this.getMultiplier());
        }
        return 0;
    }
