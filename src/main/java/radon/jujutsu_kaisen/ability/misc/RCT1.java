...
public class RCT1 extends Ability implements Ability.IChannelened {
    ...
    public void run(LivingEntity owner) {
        owner.heal((float) (ConfigHolder.SERVER.sorcererHealingAmount.get().floatValue() * Math.pow(1.75D, this.getPower(owner))));
    }

    @Override
    public float getCost(LivingEntity owner) {
        if (owner.getHealth() < owner.getMaxHealth()) {
            return (float) (ConfigHolder.SERVER.sorcererHealingAmount.get().floatValue() * Math.pow(1.75D, this.getPower(owner)) * this.getMultiplier());
        }
        return 0;
    }
