...
public class MaximumRedRenderer extends EntityRenderer<MaximumRedBeam> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(JujutsuKaisen.MOD_ID, "textures/entity/maximum_red.png");
    ...
    public MaximumRedRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull MaximumRedBeam pEntity) {
        return TEXTURE;
    }

    @Override
    public void render(MaximumRedBeam pEntity, float pEntityYaw, float pPartialTick, @NotNull PoseStack pPoseStack, @NotNull MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (frame < 0) {
            frame = MaximumRedBeam.FRAMES * 2;
        }

        pPoseStack.pushPose();
        pPoseStack.scale(MaximumRedBeam.SCALE, MaximumRedBeam.SCALE, MaximumRedBeam.SCALE);
        ...
        if (pEntity.getTime() > MaximumRedBeam.CHARGE) {
