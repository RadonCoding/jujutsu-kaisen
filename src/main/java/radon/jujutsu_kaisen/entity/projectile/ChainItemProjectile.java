...
public class ChainItemProjectile extends AbstractArrow {
    private static final float MAX_SPEED = 5.0F;

    private boolean released;
    private boolean dealtDamage;

    private static final EntityDataAccessor<ItemStack> DATA_ITEM = SynchedEntityData.defineId(ChainItemProjectile.class, EntityDataSerializers.ITEM_STACK);
    private static final EntityDataAccessor<Integer> DATA_TIME = SynchedEntityData.defineId(ChainItemProjectile.class, EntityDataSerializers.INT);

    public ChainItemProjectile(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);

        this.noCulling = true;
    }

    public ChainItemProjectile(LivingEntity pShooter, ItemStack stack) {
        super(JJKEntities.CHAIN_ITEM.get(), pShooter, pShooter.level);
        ...
        if (this.getOwner() instanceof LivingEntity owner) {
            if (!this.released) {
                if (owner.isUsingItem()) {
                    ...
                    this.setDeltaMovement(owner.getLookAngle().scale(Math.min(MAX_SPEED, Math.toRadians(this.getTime() * this.getTime()))));
                    this.released = true;
                }
            } else {
                super.tick();
