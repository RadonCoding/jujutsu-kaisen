...
public class ProjectionFrameEntity extends Entity {
    ...
    public ProjectionFrameEntity(LivingEntity owner, LivingEntity target) {
        super(JJKEntities.PROJECTION_FRAME.get(), target.level());
        ...
        this.moveTo(target.getX(), target.getY() + (target.getBbHeight() / 2.0F), target.getZ(), target.getYRot(), target.getXRot());
    }
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide && (victim == null || victim.isRemoved() || !victim.isAlive())) {
            ...
            if (this.getTime() >= DURATION) {
                ...
                if (this.pos != null) {
                    victim.moveTo(this.pos.x(), this.pos.y(), this.pos.z(), this.getYRot(), this.getXRot());
